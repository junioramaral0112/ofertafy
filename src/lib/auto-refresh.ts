...
async function scrapeAmazonPromos(): Promise<PromoDeal[]> {
  ...
  // Amazon — usa o scraper existente (Puppeteer), mesmo sem ASSOCIATE_TAG
  try {
...
async function scrapeShopeePromos(): Promise<PromoDeal[]> {
  ...
  // Shopee — usa o scraper existente, mesmo sem API keys
  try {
    const { fetchShopeeDeals } = await import('./affiliates/shopee')
    const config = {
      shopeeAppId: process.env.SHOPEE_APP_ID || 'default',
      shopeeSecret: process.env.SHOPEE_SECRET || 'default',
    }
